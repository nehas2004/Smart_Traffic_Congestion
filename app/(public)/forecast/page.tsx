...
// ONE-LINE SWAP: replace with '/traffic/forecast' when Shadeed's API is ready
...
export default function ForecastPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setCorridors(safe.sort((a, b) => b.predicted_congestion - a.predicted_congestion))
  ...
  function trendLabel() {
    ...
    if (delta > 10) return 'Traffic expected to worsen significantly over the next 15 minutes'
    ...
    <div style={{ minHeight: '100vh', background: '#faf8f5', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <header style={{
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 20px', height: 60,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 12, color: '#a67c52', display: 'flex', alignItems: 'center', gap: 5 }}>
              <Radio size={12} /> {lastUpdated || '—'}
            </span>
      ...
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '36px 20px 80px' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontSize: 'clamp(22px, 4vw, 30px)', fontWeight: 900, color: '#2c2825', margin: '0 0 8px', letterSpacing: '-0.5px' }}>
            Traffic Forecast — Next 15 Minutes
          </h1>
          <p style={{ fontSize: 14, color: '#9e9189', margin: 0 }}>
            Simplified public forecast based on live sensor data and ML predictions. Updated every 5 minutes.
          </p>
        </div>

        {loading ? (
          <div style={{ textAlign: 'center', padding: 60, color: '#9e9189', fontSize: 14 }}>Loading forecast data…</div>
        ) : (
          <>
            {/* City-wide summary card */}
            {avgNow !== null && avgNext !== null && (
              <div style={{
                background: '#2c2825', borderRadius: 18, padding: '24px 28px', marginBottom: 28,
            ...
            {/* Per-corridor full forecast */}
            <h2 style={{ fontSize: 15, fontWeight: 800, color: '#2c2825', margin: '0 0 14px' }}>Corridor-by-Corridor Breakdown</h2>
